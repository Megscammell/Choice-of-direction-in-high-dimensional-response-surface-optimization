...
def test_1():
    """
    Numerical test for quad_f().
    """
    x = np.array([2, 2])
    t = np.array([7.5, 9])
    A = np.array([[1, 0],
                  [0, 4]])
    func_val = est_dir.quad_f(x, t, A)
...
def test_2(m):
    """
    Series of tests for quad_f_noise() to check oututs are of correct
    form.
    """
    ...
    mu = 0
    sd = 1
    func_val = est_dir.quad_f_noise(x, t, A, mu, sd)
