...
def check_tolerance(s, const):
    ...
    count : integer
            Number of eigenvalues greater than tolerance.
    """
    tol = np.max(s) * const
...
def mean_no_eigs_removed(count_eigs_removed, const_list, n_list):
    ...
    for j in range(len(const_list)):
        for i in range(len(n_list)):
            if i == 0:
                assert(np.all(count_eigs_removed[j, i] == 50))
            elif i == 2:
                assert(np.all(count_eigs_removed[j, i] == 101))
...
if __name__ == "__main__":
    ...
    m = 100
    n_list = [50, 100, 200]
    const_list = [0.000000000000001, 0.001, 0.01, 0.15]
    ...
    for const in const_list:
        ...
        for n in n_list:
            ...
            for j in range(100):
                ...
                u, s, vh = np.linalg.svd(full_act_design)
