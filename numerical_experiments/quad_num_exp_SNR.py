...
if __name__ == "__main__":
    ...
    time_taken_MP = (np.genfromtxt(
                    'time_taken_MP_n=%s_m=%s_lambda_max=%s_%s_%s'
                    '_%s_%s_%s.csv' %
                    (n, m, lambda_max,
                     no_vars,
                     type_inverse,
    ...
    time_taken_XY = (np.genfromtxt(
                     'time_taken_XY_n=%s_m=%s_lambda_max=%s_%s_%s_%s_%s.csv' %
                     (n, m, lambda_max,
                      no_vars,
                      region,
                      function_type,
    ...
    if store_max_func_evals is None:
        range_2_norm_all = max(np.max(fp_norms_LS/sp_norms),
                               np.max(fp_norms_MP/sp_norms),
                               np.max(fp_norms_XY/sp_norms))
        range_2_func_all = max(np.max(fp_func_vals_LS/sp_func_vals),
                               np.max(fp_norms_MP/sp_norms),
    ...
    # range_2_func = max(np.max(time_taken_MP), np.max(time_taken_XY))
    # create_boxplots_ratio_2(time_taken_MP,
    #                         time_taken_XY,
    #                         labels[4], m, n, lambda_max, 'time_MP_XY',
    #                         snr_list, no_vars, -0.01, range_2_func,
    #                         region, function_type, save_outputs)
